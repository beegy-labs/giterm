...
> Terminal IME for iOS WKWebView | **Last Updated**: 2026-03-12
...
- 50ms fallback timer → executes all actions via `executeActions()` (incl. swap → resetField)
- Enter / toolbar backspace with composing → `resetField()` directly
- onChange fallback → `resetField()` if beforeinput missed

> **v0.2 fix**: Timer callback previously only set `needsResetRef=true` for swap actions,
> deferring reset to next keystroke. This caused backspace to be consumed by the deferred
> reset instead of deleting from terminal. Fix: timer now calls `executeActions(actions, false)`
> for all action types — safe because iOS in-flight events are gone after 50ms.
