...
impl SshSessionManager {
    ...
    async fn connect_inner(
        ...
        // Set LANG for UTF-8 support (Korean, Japanese, etc.)
        // Many servers reject set_env (AcceptEnv not configured), so ignore errors.
    ...
    /// Disconnect ALL active sessions. Called on app exit.
    pub async fn disconnect_all(&self) {
        ...
        for id in sessions {
            if let Err(e) = self.disconnect(&id).await {
                log::error!("Failed to disconnect session {}: {}", id, e);
            }
        }
    }
